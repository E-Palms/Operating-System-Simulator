...
void concatenateString( char *destStr, const char *sourceStr)
   {
    ...
    while(tempStr[sourceIndex] != NULL_CHAR && destIndex < MAX_STR_LEN)
      {
       // assign charcaters to end of destination string
    ...
    while (masterIndex < testStrLen)
      {
       ...
       // loop to end of test string while test string/sub chars are the same
       ...
       // end internal comparision loop
       
       // increment current beggining location index
       masterIndex++;
      }
...
bool getStringConstrained( FILE *inStream, bool clearLeadingNonPrintable,
                          ...
                          char delimiter, char *capturedString )
   {
    ...
    while (
           ...
           //AND continue if not printable flag set and characters are printable
...
void getSubString( char *destStr, const char *sourceStr,
                        int startIndex, int endIndex)
   {
    // initialize function/variables
       ...
       // initialize the destination index as zero
       int destIndex = 0;
