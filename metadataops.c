    ...
    if (localPtr == NULL)
      {
       ...
       // assign all three values to the newly created node
...
bool getMetaData(const char *filename,
    ...
    *opCodeDataHead = NULL;
    ...
    // check for file open failiure
    if (fileAccessPtr == NULL)
      {
       // set end state message
          // function: copyString
       copyString(endStateMsg, "Metadata file eccess error");
    ...
    // get start and end counts for later comparision
    ...
    // check for failiure of first complete op command
    ...
    while (accessResult == COMPLETE_OPCMD_FOUND_MSG)
      {
       ...
       // update start and end counts for later comparision
          // function: updateStartCount, updateEndCount
       startCount = updateStartCount(startCount, newNodePtr->strArg1);
       endCount = updateEndCount(endCount, newNodePtr->strArg1);
      }
...
OpCodeMessages getOpCommand(FILE *filePtr, OpCodeType *inData)
   {
    // initialize function/variables
       ...
       const int MAX_ARG_STRING_LENGTH = 15;

       // initialize other variables
       int numBuffer = 0;
       char strBuffer[STD_STR_LEN];
       char cmdBuffer[MAX_CMD_LENGTH];
       char argStrBuffer[MAX_ARG_STRING_LENGTH];
       int runningStringIndex = 0;
       bool arg2FailiureFlag = false;
       bool arg3FailiureFlag = false;
    ...
    // otherwise, assume unsuccessful access
    else
      {
       ...
       // return op command access failiure
    ...
    // check int args for upload failiure
    if (arg2FailiureFlag || arg3FailiureFlag)
      {
       // return corrupt op command found message
       return CORRUPT_OPCMD_ARG_ERR;
      }
